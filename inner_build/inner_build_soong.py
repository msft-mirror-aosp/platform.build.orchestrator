...
from find_api_packages import BazelLabel, ApiPackageFinder
import common
...
class InnerBuildSoong(common.Commands):
    ...
    def export_api_contributions(self, args):
        # Bazel is used to export API contributions even when the primary build system is Soong
...
class ApiMetadataFile(object):
    ...
    def newerthan(self, otherpath: str) -> bool:
        ...
        return not os.path.exists(otherpath) or os.path.getmtime(otherpath) < os.path.getmtime(self.fullpath())

class ApiExporterBazel(object):
    """Class to generate API surface metadata files into a well-known directory
    This directory is subsequently scanned by the build orchestrator for API surface assembly"""
    def __init__(self, inner_tree: str, out_dir: str, api_domains: List[str]):
        """
        Arguments
            inner_tree: Root of the exporting tree
            out_dir: out_dir for export. The files will be copied to $our_dir/api_contribtutions
    ...
    def export_api_contributions(self):
        ...
        metadata_files = self._build_api_domain_contribution_targets(contribution_targets)
    ...
    def _find_api_domain_contribution_targets(self) -> List[str]:
        """Returns the label of the Bazel contribution targets to build
        """
        print(f"Finding api_domain_contribution Bazel BUILD targets in tree rooted at {self.inner_tree}")
    ...
    def _build_api_domain_contribution_targets(self, contribution_targets: List[str]) -> List[ApiMetadataFile]:
        """Builds the contribution targets and returns the filepath of the generated files"""
        print(f"Running Bazel build on api_domain_contribution targets in tree rooted at {self.inner_tree}")
        if not contribution_targets:
            return None
        self._run_bazel_cmd(subcmd="build",
                            targets=contribution_targets)
        print(f"Running Bazel cquery on api_domain_contribution targets in tree rooted at {self.inner_tree}")
        proc = self._run_bazel_cmd(subcmd="cquery",
                                   targets=contribution_targets,
                                   subcmd_options=["--output=starlark", "--starlark:expr=' '.join([f.path for f in target.files.to_list()])"])
        filepaths = proc.stdout.decode().rstrip() # Remove trailing newline
        filepaths = filepaths.split(" ") # Covert to array
        return [ApiMetadataFile(inner_tree=self.inner_tree, path=filepath) for filepath in filepaths]

    def _copy_api_domain_contribution_metadata_files(self, files: List[ApiMetadataFile]):
        """Copies the metadata files to a well-known location"""
        target_dir = os.path.join(self.out_dir, "api_contributions")
        print(f"Copying API contribution metadata files of tree rooted at {self.inner_tree} to {target_dir}")
        # Create the directory if it does not exist, even if that inner_tree has no contributions
    ...
    def _run_bazel_cmd(self, subcmd: str, targets: List[str], subcmd_options=[]) -> subprocess.CompletedProcess:
        """Runs Bazel subcmd with Multi-tree specific configuration"""
        # TODO (b/244766775): Replace the two discrete cmds once the new b-equivalent entrypoint is available
        self._run_bp2build_cmd()
        cmd = [
                "build/bazel/bazel.sh", # Android's Bazel-entrypoint. Contains configs like the JDK to use
                subcmd,
                "--config=api_bp2build", # Run Bazel on the synthetic api_bp2build workspace
                "--config=android",
        ]
    ...
    def _run_cmd(self, cmd) -> subprocess.CompletedProcess:
        proc = subprocess.run(cmd, cwd=self.inner_tree, shell=False, capture_output=True)
        if proc.returncode:
            sys.stderr.write(f"export_api_contributions cmd: {cmd} failed with error message:\n")
